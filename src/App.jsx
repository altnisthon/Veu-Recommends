...
function RichText({ text }) {
  ...
  while (i < lines.length) {
    ...
    // Detect a product block: 4 consecutive label lines (Product/Shade/Shop/Reason)
    // or at least Product: starts a block
    if (trimmed.startsWith('Product:')) {
      // Collect the block (up to 4 label lines)
      const blockLines = [];
      let j = i;
      while (j < lines.length && j < i + 6) {
        const bl = lines[j].trim();
        if (bl === '' && blockLines.length > 0) break;
        if (bl !== '') blockLines.push(bl);
        j++;
      }

      out.push(
        <div key={`prod-${i}`} style={{
          background:'#FDF8F5',
          border:'1px solid #F0E8E6',
          borderLeft:'3px solid #932D28',
          ...
          {blockLines.map((bl, bi) => {
            ...
            if (colonIdx === -1) return <span key={bi}>{bl}</span>;
            const label = bl.slice(0, colonIdx);
            const value = bl.slice(colonIdx + 1).trim();
            if (!LABELS.includes(label)) return <span key={bi}>{renderLine(bl, `bl-${i}-${bi}`)}</span>;
            return (
              <div key={bi} style={{display:'flex',gap:6,alignItems:'baseline',flexWrap:'wrap'}}>
                <span style={{
                  fontSize:9,
                  fontWeight:700,
    ...
    if (trimmed === '') {
      out.push(<div key={`sp-${i}`} style={{height:4}}/>);
      i++; continue;
    }

    // Normal text line
    out.push(
      <div key={`ln-${i}`} style={{marginBottom:2}}>
        {renderLine(trimmed, `ln-${i}`)}
      </div>
    );
    i++;
  }
